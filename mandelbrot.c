/* mandelbrot.c — fast terminal Mandelbrot explorer
 *
 ...
 *
 *  Build:
 *    gcc -O3 -march=native -funroll-loops -ffast-math -fopenmp \
 *        -o mandelbrot mandelbrot.c -lm
 */
#define _GNU_SOURCE
#include <errno.h>
#include <math.h>
...
static void term_init(void) {
    ...
    /* SIGWINCH must NOT restart read() so we detect resize quickly */
...
typedef struct { int pi; char ch; } IterEntry;  /* pi=-1 → inside set */
...
/* dark-grey escape for set interior */
...
static void render(void) {
    int W, H;
    term_size(&W, &H);
    int h = H - 1;                 /* reserve last row for status */
    if (h < 1 || W < 1) return;
    ensure_bufs(W, h);

    /* sy: coordinate units per row; sx: per column (chars ~2:1 tall:wide) */
    ...
    for (int y = 0; y < h; y++) {
        char *rb  = g_rows[y];
        int   rp  = 0;
        int   ppi = -2;            /* previous palette index (impossible init) */
        double ci = y0 + y * sy;

        for (int x = 0; x < W; x++) {
            double cr = x0 + x * sx;

            /* shortcut: main cardioid and period-2 bulb */
            double p  = cr - 0.25;
            double q  = p*p + ci*ci;
            int iter;
            if (q * (q + p) <= 0.25 * ci*ci ||
            ...
            /* emit colour escape only when it changes */
            if (e->pi != ppi) {
                if (e->pi < 0) {
    ...
    /* assemble into one frame buffer then single write() */
    int fp = 0;
    ...
    fp += snprintf(g_frame+fp, 512,
        ...
        "  \033[1mcx\033[0m=%-15.8g"
        "  \033[1mcy\033[0m=%-15.8g"
        "  \033[1mzoom\033[0m=%-12.5g"
        "  \033[1miter\033[0m=%-5d"
...
int main(void) {
    ...
    for (;;) {
        ...
        double vs = 0.25 / g_zoom;   /* vertical step   */
        double hs = 0.50 / g_zoom;   /* horizontal step */
